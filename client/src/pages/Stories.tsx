...
export default function Stories() {
  ...
  return <div className="stories-page min-h-screen bg-[#f5f1e8] text-[#17251f]" style={{ fontFamily: SANS }}>
    <style>{`
      .stories-page .wrap{width:min(1320px,calc(100% - 64px));margin:0 auto}.stories-page .display{font-family:${DISPLAY};font-weight:400;letter-spacing:.045em;line-height:.92;text-transform:uppercase}.stories-page .eyebrow{font-size:11px;font-weight:700;letter-spacing:.18em;text-transform:uppercase}.stories-page .summary{font-size:16px;line-height:1.65;letter-spacing:.025em;color:#59605b}.stories-page a{text-decoration:none;color:inherit}.stories-page .image{width:100%;height:100%;object-fit:cover;display:block}.stories-page .section-nav{display:flex;justify-content:center;gap:clamp(26px,5vw,72px);overflow-x:auto}.stories-page .section-tab{white-space:nowrap;padding:0 0 13px;border:0;border-bottom:3px solid transparent;background:none;font-size:12px;font-weight:700;letter-spacing:.14em;text-transform:uppercase}.stories-page .section-tab.active{color:#9b5e3d;border-bottom-color:#9b5e3d}.stories-page .preview-grid{display:grid;grid-template-columns:1.22fr .78fr;gap:32px;align-items:start}.stories-page .preview-grid article:nth-child(2){margin-top:105px}.stories-page .archive-grid{display:grid;grid-template-columns:repeat(12,1fr);gap:58px 32px}.stories-page .archive-card{grid-column:span 6}.stories-page .archive-card:nth-child(3n){grid-column:3/span 8}.stories-page .recommend-grid{display:grid;grid-template-columns:1.35fr .8fr .8fr;gap:28px;align-items:start}.stories-page .recommend-grid article:nth-child(2){margin-top:85px}.stories-page .button{display:inline-block;background:#111;color:#fff!important;border:2px solid #111;border-radius:3px;padding:13px 30px;font-size:12px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;transition:background .2s,color .2s}.stories-page .button:hover{background:transparent;color:#111!important}@media(max-width:767px){.stories-page .wrap{width:calc(100% - 40px)}.stories-page .section-nav{justify-content:flex-start;gap:27px}.stories-page .preview-grid,.stories-page .archive-grid,.stories-page .recommend-grid{display:block}.stories-page .preview-grid article,.stories-page .archive-card,.stories-page .recommend-grid article{margin:0 0 48px!important}}
    `}</style>
    <Navigation />
...
function StoriesOverview({ stories, currentSection, scrollToCategory, getObjectPosition }: { stories: EditorialStory[]; currentSection: StoryCategory; scrollToCategory: (category: StoryCategory) => void; getObjectPosition: (url: string) => string }) {
  return <>
    ...
    <section style={{ background: '#f5f1e8', padding: 'clamp(95px,12vw,170px) 0' }}><div className="wrap"><div className="grid md:grid-cols-[1fr_.7fr] gap-8 items-end" style={{ marginBottom: 55 }}><div><p className="eyebrow" style={{ color: '#9b5e3d', margin: '0 0 17px' }}>Selected by Tree Thousands</p><h2 className="display" style={{ fontSize: 'clamp(54px,8vw,106px)', margin: 0 }}>Recommended Stories</h2></div><p className="summary" style={{ margin: '0 0 8px' }}>Three perspectives for continuing through the people, landscapes, and everyday details behind the journey.</p></div><div className="recommend-grid">{stories.slice(1, 4).map((story, index) => <article key={story.slug}><Link href={`/stories/article/${story.slug}`}><img src={story.coverImage} alt={story.title} className="image" style={{ height: index === 0 ? 560 : 410, objectPosition: getObjectPosition(story.coverImage) }} /><p className="eyebrow" style={{ color: '#9b5e3d', margin: '20px 0 10px' }}>{story.category}</p><h3 style={{ fontSize: 'clamp(25px,2.5vw,35px)', lineHeight: 1.2, fontWeight: 500, margin: '0 0 12px' }}>{story.title}</h3><p className="summary" style={{ margin: 0 }}>{story.excerpt}</p></Link></article>)}</div></div></section>
  </>;
}
