...
export default function StoryDetail() {
  ...
  return <div className="story-detail bg-[#f5f1e8] text-[#17251f]" style={{ fontFamily: SANS }}>
    ...
    <main>
      ...
      <section className="editorial-split" style={{ background: '#e7dfd1' }}><img className="split-image" src={supportingImages[0]} alt={`A detail from ${story.location}`} style={{ width: '100%', height: 680, objectFit: 'cover', display: 'block', objectPosition: getObjectPosition(supportingImages[0]) }} /><div className="split-copy flex flex-col justify-center" style={{ padding: 'clamp(55px,7vw,110px)' }}><p className="eyebrow" style={{ color: '#9b5e3d', margin: '0 0 24px' }}>Inside the Story</p><h2 className="display" style={{ fontSize: 'clamp(46px,5.5vw,76px)', margin: '0 0 30px' }}>{supplement.label}</h2><p style={{ fontSize: 18, lineHeight: 1.78, letterSpacing: '.025em', color: '#424b45', margin: '0 0 28px' }}>{supplement.context}</p>{prose[2] && <p style={{ fontSize: 18, lineHeight: 1.78, letterSpacing: '.025em', color: '#424b45', margin: 0 }}>{prose[2]}</p>}</div></section>

      <section className="quote-split" style={{ background: '#17352d', color: '#f5f1e8' }}><blockquote className="split-copy flex flex-col justify-center" style={{ margin: 0, padding: 'clamp(55px,7vw,105px)' }}><p className="eyebrow" style={{ color: '#c79a72', margin: '0 0 25px' }}>A line to remember</p><p style={{ fontSize: 'clamp(28px,3.5vw,48px)', lineHeight: 1.2, letterSpacing: '-.02em', margin: 0 }}>“{quote}”</p></blockquote><img className="split-image" src={supportingImages[1]} alt={`Life around ${story.location}`} style={{ width: '100%', height: 620, objectFit: 'cover', display: 'block', objectPosition: getObjectPosition(supportingImages[1]) }} /></section>
      ...
      <section className="editorial-split" style={{ background: '#fff' }}><div className="split-copy flex flex-col justify-center" style={{ padding: 'clamp(55px,7vw,110px)' }}><p className="eyebrow" style={{ color: '#9b5e3d', margin: '0 0 25px' }}>Closing Reflection</p><p style={{ fontSize: 'clamp(27px,3.2vw,44px)', lineHeight: 1.28, letterSpacing: '-.015em', margin: '0 0 38px' }}>{supplement.reflection}</p><div className="prose">{prose.slice(3).map((paragraph, index) => <p key={index}>{paragraph}</p>)}</div></div><img className="split-image" src={supportingImages[2]} alt={`A wider view of ${story.location}`} style={{ width: '100%', height: 720, objectFit: 'cover', display: 'block', objectPosition: getObjectPosition(supportingImages[2]) }} /></section>
