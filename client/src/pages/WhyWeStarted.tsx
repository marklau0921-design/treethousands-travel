  ...
  return (
    <div className="our-story-detail bg-[#f4f0e7] text-[#1b241f]">
      <style>{`
        ...
        .our-story-detail .pillar.layout-1,.our-story-detail .pillar.layout-4{grid-column:span 7}
        .our-story-detail .pillar.layout-2,.our-story-detail .pillar.layout-3{grid-column:span 5}
        .our-story-detail .pillar.layout-2{margin-top:110px}
        .our-story-detail .pillar-image{width:100%;height:clamp(360px,44vw,620px);object-fit:cover;display:block}
        .our-story-detail .pillar.layout-2 .pillar-image,.our-story-detail .pillar.layout-3 .pillar-image{height:clamp(330px,35vw,500px)}
        @media(max-width:767px){.our-story-detail .editorial-wrap{width:calc(100% - 40px)}.our-story-detail .split{grid-template-columns:1fr}.our-story-detail .body-copy{font-size:16px;line-height:1.65}.our-story-detail .pillar-grid{display:block}.our-story-detail .pillar{margin:0 0 48px!important}.our-story-detail .pillar-image,.our-story-detail .pillar.layout-2 .pillar-image,.our-story-detail .pillar.layout-3 .pillar-image{height:360px}}
      `}</style>
      <Navigation />

      <header className="relative flex items-end overflow-hidden" style={{ minHeight: 'min(900px, 92vh)', background: '#19372f' }}>
        {page.hero.image && <img src={page.hero.image} alt={page.hero.title} className="absolute inset-0 w-full h-full object-cover" onError={event => { event.currentTarget.style.display = 'none'; }} style={{ objectPosition: getObjectPosition(page.hero.image) }} />}
      ...
      <section className="split" style={{ background: page.quote.backgroundColor, color: '#f4f0e7' }}><div className="flex flex-col justify-center" style={{ padding: 'clamp(70px,9vw,140px)' }}><p className="eyebrow" style={{ color: '#c79a72', margin: '0 0 34px' }}>{page.quote.eyebrow}</p><blockquote style={{ fontFamily: SANS, fontSize: 'clamp(38px,5vw,72px)', letterSpacing: '-.025em', lineHeight: 1.08, margin: 0 }}>“{page.quote.quote}”</blockquote><p className="body-copy" style={{ color: 'rgba(244,240,231,.72)', maxWidth: 610, margin: '40px 0 0' }}>{page.quote.body}</p></div>{page.quote.image && <img src={page.quote.image} alt={page.quote.eyebrow} className="w-full h-full object-cover" onError={event => { event.currentTarget.style.display = 'none'; }} style={{ minHeight: 700, objectPosition: getObjectPosition(page.quote.image) }} />}</section>

      <section style={{ padding: 'clamp(100px,12vw,170px) 0', background: page.pillars.backgroundColor }}><div className="editorial-wrap"><div style={{ maxWidth: 760, marginBottom: 'clamp(64px,8vw,110px)' }}><p className="eyebrow" style={{ color: '#8b5a3c', margin: '0 0 22px' }}>{page.pillars.eyebrow}</p><h2 className="display-title" style={{ fontSize: 'clamp(46px,6vw,82px)', color: '#17352d', margin: '0 0 28px' }}>{page.pillars.title}</h2><p className="body-copy" style={{ maxWidth: 650, margin: 0 }}>{page.pillars.intro}</p></div><div className="pillar-grid">{page.pillars.items.map((pillar, index) => <figure className={`pillar layout-${pillar.layout ?? ((index % 4) + 1)}`} key={`${pillar.title}-${index}`} style={{ margin: 0 }}><img src={pillar.image} alt={pillar.title} className="pillar-image" style={{ objectPosition: getObjectPosition(pillar.image) }} /><figcaption style={{ paddingTop: 22 }}><h3 className="display-title" style={{ fontSize: 'clamp(32px,3vw,44px)', color: '#17352d', margin: '0 0 9px' }}>{pillar.title}</h3><p className="body-copy" style={{ fontSize: 15, maxWidth: 520, margin: 0 }}>{pillar.text}</p></figcaption></figure>)}</div></div></section>

      {page.extraSections.map(section => {
        if (section.type === 'quote') return <section key={section.id} className="split" style={{ background: section.backgroundColor, color: '#f4f0e7' }}><div className="flex flex-col justify-center" style={{ padding: 'clamp(70px,9vw,140px)' }}><p className="eyebrow" style={{ color: '#d6b18d', margin: '0 0 34px' }}>{section.eyebrow}</p><blockquote style={{ fontFamily: SANS, fontSize: 'clamp(38px,5vw,72px)', letterSpacing: '-.025em', lineHeight: 1.08, margin: 0 }}>“{section.quote}”</blockquote><p className="body-copy" style={{ color: 'rgba(244,240,231,.76)', maxWidth: 610, margin: '40px 0 0' }}>{section.body}</p></div>{section.image && <img src={section.image} alt={section.eyebrow} className="w-full h-full object-cover" style={{ minHeight: 700, objectPosition: getObjectPosition(section.image) }} />}</section>;
        if (section.type === 'pillars') return <section key={section.id} style={{ padding: 'clamp(100px,12vw,170px) 0', background: section.backgroundColor }}><div className="editorial-wrap"><div style={{ maxWidth: 760, marginBottom: 'clamp(64px,8vw,110px)' }}><p className="eyebrow" style={{ color: '#8b5a3c', margin: '0 0 22px' }}>{section.eyebrow}</p><h2 className="display-title" style={{ fontSize: 'clamp(46px,6vw,82px)', color: '#17352d', margin: '0 0 28px' }}>{section.title}</h2><p className="body-copy" style={{ maxWidth: 650, margin: 0 }}>{section.intro}</p></div><div className="pillar-grid">{section.items.map((item, index) => <figure className={`pillar layout-${item.layout ?? ((index % 4) + 1)}`} key={`${section.id}-${index}`} style={{ margin: 0 }}>{item.image && <img src={item.image} alt={item.title} className="pillar-image" style={{ objectPosition: getObjectPosition(item.image) }} />}<figcaption style={{ paddingTop: 22 }}><h3 className="display-title" style={{ fontSize: 'clamp(32px,3vw,44px)', color: '#17352d', margin: '0 0 9px' }}>{item.title}</h3><p className="body-copy" style={{ fontSize: 15, maxWidth: 520, margin: 0 }}>{item.text}</p></figcaption></figure>)}</div></div></section>;
        if (section.type === 'closing') return <section key={section.id} className="split" style={{ background: section.backgroundColor }}>{section.image && <img src={section.image} alt={section.title} className="w-full h-full object-cover" style={{ minHeight: 650, objectPosition: getObjectPosition(section.image) }} />}<div className="flex flex-col justify-center" style={{ padding: 'clamp(60px,8vw,120px)' }}><p className="eyebrow" style={{ color: '#74482f', margin: '0 0 24px' }}>{section.eyebrow}</p><h2 className="display-title" style={{ fontSize: 'clamp(48px,5.4vw,78px)', color: '#17352d', margin: '0 0 30px' }}>{section.title}</h2><div className="body-copy" style={{ color: '#3f4741' }}>{section.paragraphs.map((paragraph, index) => <p key={index} style={{ margin: index === section.paragraphs.length - 1 ? 0 : '0 0 20px' }}>{paragraph}</p>)}</div></div></section>;
        return <section key={section.id} style={{ padding: 'clamp(90px,11vw,160px) 0' }}><div className="editorial-wrap split" style={{ background: section.backgroundColor }}><div className="flex flex-col justify-center" style={{ padding: 'clamp(42px,6vw,90px)' }}><p className="eyebrow" style={{ color: '#8b5a3c', margin: '0 0 22px' }}>{section.eyebrow}</p><h2 className="display-title" style={{ fontSize: 'clamp(45px,5vw,72px)', color: '#172d25', margin: '0 0 30px' }}>{section.title}</h2><div className="body-copy">{section.paragraphs.map((paragraph, index) => <p key={index} style={{ margin: index === section.paragraphs.length - 1 ? 0 : '0 0 20px' }}>{paragraph}</p>)}</div></div>{section.image && <img src={section.image} alt={section.title} className="w-full h-full object-cover" style={{ minHeight: 660, objectPosition: getObjectPosition(section.image) }} />}</div></section>;
      })}
