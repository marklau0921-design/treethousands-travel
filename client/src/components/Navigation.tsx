...
export default function Navigation({ forceHide = false }: NavigationProps) {
  ...
  const contactTexture = homepageAssets?.cta?.url || '';
  const contactTextureOpacity = Math.max(0, Math.min(1, Number(homepageAssets?.cta?.opacity ?? 18) / 100));
  ...
  return <>
    <style>{`
      ...
      .tt-contact-grid{display:grid;grid-template-columns:1fr 1fr;min-height:calc(100vh - 55px)}.tt-contact-choice{position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;padding:clamp(50px,8vw,125px);text-decoration:none;transition:filter .2s}.tt-contact-choice:hover{filter:brightness(.92)}.tt-contact-content{position:relative;z-index:1}@media(max-width:767px){.tt-contact-grid{grid-template-columns:1fr;min-height:calc(100vh - 55px)}.tt-contact-choice{min-height:45vh;padding:48px 28px}}
    `}</style>
    ...
    {activeItem && <div style={{ position: 'fixed', inset: '55px 0 0', background: '#fff', zIndex: 49, overflow: 'auto' }}>
      ...
      {activeMenu === 'join-us' ? <div className="tt-contact-grid"><a className="tt-contact-choice" href={`mailto:${contactSettings?.email || ''}`} onClick={() => setActiveMenu(null)} style={{backgroundColor:contactSettings?.emailBackgroundColor||'#f5f1e8',color:contactSettings?.emailTextColor||'#17251f'}}>{contactTexture&&<span aria-hidden style={{position:'absolute',inset:0,backgroundImage:`url(${contactTexture})`,backgroundSize:'cover',backgroundPosition:'center',opacity:contactTextureOpacity}}/>}<div className="tt-contact-content"><Mail size={30} strokeWidth={1.5}/><p style={{fontFamily:NAV_SANS,fontSize:11,fontWeight:700,letterSpacing:'.18em',textTransform:'uppercase',margin:'28px 0 16px'}}>Write to us</p><h2 style={{fontFamily:NAV_DISPLAY,fontSize:'clamp(48px,6vw,86px)',fontWeight:400,letterSpacing:'.04em',lineHeight:.95,textTransform:'uppercase',margin:'0 0 20px'}}>Email</h2><p style={{fontFamily:NAV_SANS,fontSize:15,lineHeight:1.7,maxWidth:430,opacity:.72,margin:0}}>Share your ideas and travel questions in your own time. We’ll reply personally.</p>{contactSettings?.email&&<span style={{display:'block',fontFamily:NAV_SANS,fontSize:13,marginTop:24,textDecoration:'underline'}}>{contactSettings.email}</span>}</div></a><a className="tt-contact-choice" href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noreferrer" onClick={() => setActiveMenu(null)} style={{backgroundColor:contactSettings?.whatsappBackgroundColor||'#e5ddce',color:contactSettings?.whatsappTextColor||'#17251f'}}>{contactTexture&&<span aria-hidden style={{position:'absolute',inset:0,backgroundImage:`url(${contactTexture})`,backgroundSize:'cover',backgroundPosition:'center',opacity:contactTextureOpacity}}/>}<div className="tt-contact-content"><MessageCircle size={30} strokeWidth={1.5}/><p style={{fontFamily:NAV_SANS,fontSize:11,fontWeight:700,letterSpacing:'.18em',textTransform:'uppercase',margin:'28px 0 16px'}}>Talk with us</p><h2 style={{fontFamily:NAV_DISPLAY,fontSize:'clamp(48px,6vw,86px)',fontWeight:400,letterSpacing:'.04em',lineHeight:.95,textTransform:'uppercase',margin:'0 0 20px'}}>WhatsApp</h2><p style={{fontFamily:NAV_SANS,fontSize:15,lineHeight:1.7,maxWidth:430,opacity:.72,margin:0}}>Start a direct conversation for quick questions, introductions, or early journey ideas.</p></div></a></div> : <div style={{ paddingTop: 60, paddingBottom: 40, paddingLeft: 'clamp(28px,calc(-645px + 49.82vw),305px)', paddingRight: 64, maxWidth: 760 }}>
